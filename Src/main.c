...
uint16_t captureEDGE[4]={1};  //0==RISINGEDGE  1=FALLINGEDGE
...
int main(void)
{
  ...
  motor_start(0);
	...
	Servo_SetAngle(0,92.1);
	Servo_SetAngle(2,180);
	
	//compile
	
	//Servo_SetAngle(1,270);
	//Servo_SetAngle(2,180);
	
	/*
	for(float temp=50;temp<=250;temp+=30)
	{
...
//flag[5]=1;
  ...
  while (1)
  {
    ...
    /* USER CODE BEGIN 3 */
   //motor[0].MOTOR_DIE;motor[1].MOTOR_DIE;motor[2].MOTOR_DIE;motor[3].MOTOR_DIE;
		//motor[0].motor_pid.target;motor[1].motor_pid.target;motor[2].motor_pid.target;motor[3].motor_pid.target;
		if(flag[5]==1)
		{
		     //HAL_GPIO_TogglePin(GPIOC,GPIO_PIN_13);
			   ...
			   if(fabs(data_2.data)<0.05)  data_2.data=0;
					car_speed[0]=data_2.data*1024;  //x  
		...
		//(int)motor_w[0])/10*10
		                ...
		                if(servo_mode==1)//DOWN
                    {
                        //printf("A\n");
												Servo_SetAngle(1,168);
											  HAL_Delay(600);
										  	Servo_SetAngle(0,40);
												Servo_SetAngle(2,180);
												HAL_Delay(900);
												Servo_SetAngle(2,165);							
                    }
										if(servo_mode==2)//LEFT
                    {
                        //printf("B\n");
                        Servo_SetAngle(0,92.1);
												Servo_SetAngle(1,5);
	   ...
	   if(flag[0])
	   {
			 ...
			 {
				 __HAL_TIM_SET_CAPTUREPOLARITY(EncoderHandle[0], EncoderChannel[0][0], TIM_INPUTCHANNELPOLARITY_FALLING); //?is it necessary?
				 captureEDGE[0]--;
			 }
			 else
			 {
				 __HAL_TIM_SET_CAPTUREPOLARITY(EncoderHandle[0], EncoderChannel[0][0], TIM_INPUTCHANNELPOLARITY_RISING);
				 captureEDGE[0]++;
			 }
		   cnt[0]++;
		  ...
		  if(flag[1])
	   {
	    	if(captureEDGE[1])   //DOUBLE FREQUENCY TEST
			 {
				 __HAL_TIM_SET_CAPTUREPOLARITY(EncoderHandle[1], EncoderChannel[1][0], TIM_INPUTCHANNELPOLARITY_FALLING); //?is it necessary?
				 captureEDGE[1]--;
			 }
			 else
			 {
				 __HAL_TIM_SET_CAPTUREPOLARITY(EncoderHandle[1], EncoderChannel[1][0], TIM_INPUTCHANNELPOLARITY_RISING);
				 captureEDGE[1]++;
			 }
		   cnt[1]++;
		   HAL_TIM_IC_Start_IT(EncoderHandle[1],EncoderChannel[1][0]);
			...
			if(flag[2])
	   {
	    	if(captureEDGE[2])   //DOUBLE FREQUENCY TEST
			 {
				 __HAL_TIM_SET_CAPTUREPOLARITY(EncoderHandle[2], EncoderChannel[2][0], TIM_INPUTCHANNELPOLARITY_FALLING); //?is it necessary?
				 captureEDGE[2]--;
			 }
			 else
			 {
				 __HAL_TIM_SET_CAPTUREPOLARITY(EncoderHandle[2], EncoderChannel[2][0], TIM_INPUTCHANNELPOLARITY_RISING);
				 captureEDGE[2]++;
			 }
		   cnt[2]++;
		 ...
		 if(flag[3])
	   {
	    	if(captureEDGE[3])   //DOUBLE FREQUENCY TEST
			 {
				 __HAL_TIM_SET_CAPTUREPOLARITY(EncoderHandle[3], EncoderChannel[3][0], TIM_INPUTCHANNELPOLARITY_FALLING); //?is it necessary?
				 captureEDGE[3]--;
			 }
			 else
			 {
				 __HAL_TIM_SET_CAPTUREPOLARITY(EncoderHandle[3], EncoderChannel[3][0], TIM_INPUTCHANNELPOLARITY_RISING);
				 captureEDGE[3]++;
			 }
		   cnt[3]++;
