...
void motor_pid_control(motor_data *motor_,uint16_t i,uint16_t actualval)
{
    if(motor_->is_motor_enable==1)   //电机使能
    {
			  float current_val = 0;      //PID 计算值（初始化为0）
				motor_->motor_overflow_counter.Capture_Count=actualval;  
        motor_->actual_speed=motor_->motor_overflow_counter.Capture_Count;
...
//        }			//11.25 放主函数去了
        ...
        if(current_val<0) 
				{
					if(motor_->motor_pid.target>5)
					{
						if(motor_->MOTOR_DIE) motor_->MOTOR_DIE=0;
					  else motor_->MOTOR_DIE=1;
